...
export async function searchPlayer(username: string): Promise<WoTPlayerStats> {
  ...
  const tanksUrl = `${BASE_URL}/account/tanks/?application_id=${APP_ID}&account_id=${account_id}`
  ...
  if (res.ok) {
    ...
    const tankList: Array<{ tank_id: number; statistics: { battles: number; wins: number; damage_dealt: number; frags: number; spotted: number; dropped_capture_points: number } }> =
      tanksData.data?.[account_id] ?? []

    // Fetch expected values for WN8
    let expData: Awaited<ReturnType<typeof useWn8['fetchExpectedValues']>> | null = null
    try {
      expData = await useWn8().fetchExpectedValues()
    } catch {
      // WN8 per vehicle won't be available
    }
    ...
    vehicles = tankList
      .filter((t) => t.statistics.battles > 0)
      .map((t) => {
  ...
  // Compute overall WN8 using aggregate stats
  let overallWn8 = 0
  if (battles > 0) {
    // Use average expected values across all vehicles as approximation
    const rawStats: VehicleRawStats = {
      battles,
      wins,
      damage_dealt,
      frags: allStats.frags ?? 0,
      spotted: allStats.spotted ?? 0,
      dropped_capture_points: allStats.dropped_capture_points ?? 0,
    }
    // Default expected values for overall calculation
  ...
  const result: WoTPlayerStats = {
    ...
    avgDamage: battles > 0 ? damage_dealt / battles : 0,
    wn8: overallWn8,
    vehicles,
    cachedAt: Date.now(),
  }
